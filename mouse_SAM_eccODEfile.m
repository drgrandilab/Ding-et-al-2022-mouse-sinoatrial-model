...
ISO_CCh_flag = p(3); % flag ISO (1) or AcH (2)
...
par_SA = p(9:end); % sensitivity analysis parameters
...
if ISO_CCh_flag == 1
    ...
    gcal13 = gcal13*2; % Larson 2013 +100%
    %v_cal unchanged % Larson 2013
    gcat = 1.75*gcat; % Larson 2013 +75%
    %kmnap = kmnap*0.85;
    koca = koca*2; % paper
    pumpkmf = pumpkmf/2; % paper
    ...
    gto = gto*1.15;
    gk1 = gk1*0.85;
    gna_ttxr = gna_ttxr*1.15; % INa1.5
elseif ISO_CCh_flag == 2
    CCh = 500; % [nM] % IK,ACh from Arbel-Ganon et al. 2020
end
%% Model equations
